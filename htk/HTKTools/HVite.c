...
Boolean ProcessFile(char *fn, Network *net, int utterNum, LogDouble currGenBeam, Boolean restartable)
{
   ...
   printf("Value of enableOutput = %d numToks = %d latExt = %s\n", enableOutput, nToks, latExt);
   ...
   if (enableOutput){
      if (latExt!=NULL) {
         MakeFN(thisFN,labDir,latExt,lfn);
         printf("Writing lattice to %s\n", lfn);
         if ((file=FOpen("outputLattice",NetOFilter,&isPipe))==NULL) 
...
void DoAlignment(void)
{
   ...
   while (NumArgs()>0) {
      ...
      if (loadNetworks) {
         ...
         FClose(nf,isPipe);
         if (trace&T_TOP) {
            printf("Read lattice %s with %d nodes / %d arcs\n",
                   lfn, wdNet->nn,wdNet->na);
            fflush(stdout);
         }
      }
      else {
